...
static inline void * fd_asan_poison  ( void * addr, ulong sz ) { printf( "poison %p\n", addr); __asan_poison_memory_region  ( addr, sz ); fd_asan_check_watch( 1, addr, sz ); return addr; }
static inline void * fd_asan_unpoison( void * addr, ulong sz ) { printf( "unpoison %p\n", addr);__asan_unpoison_memory_region( addr, sz ); fd_asan_check_watch( 0, addr, sz ); return addr; }

#else
static inline void * fd_asan_poison  ( void * addr, ulong sz ) { printf( "poison %p\n", addr);__asan_poison_memory_region  ( addr, sz ); return addr; }
static inline void * fd_asan_unpoison( void * addr, ulong sz ) { printf( "unpoison %p\n", addr);__asan_unpoison_memory_region( addr, sz ); return addr; }
#endif
